...
) : CharacterGenerator(1), _dirty_level(dirty_level), _frame(NULL), _old_frame(NULL) {
...
void SvgSubprocessCharacterGenerator::paint_gl( ) {
	MutexLock lock(mut);
	if (_frame != NULL) {
		float alpha = (float) (_frame->global_alpha()) / 255.0f;
		((CairoFrame*) _frame)->paint_gl(_x, _y, alpha);
	}
}
