...
void CairoFrame::paint_gl( int x, int y, float alpha ) {
	unsigned char* p = cairo_image_surface_get_data(crs);
	glRasterPos3f(x, y, 0.0f);
	glPixelTransferf(GL_ALPHA_SCALE, alpha);
	glPixelZoom(1.0f, -1.0f);
	glDrawPixels(_w, _h, GL_BGRA_EXT, GL_UNSIGNED_BYTE, p);
}

CairoFrame::~CairoFrame( ) {
    cairo_surface_destroy(crs);
//    printf("CairoFrame destroyed: %p\n", this);
}
